...
type RestOf<T extends IReceipt | NewReceipt> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class ReceiptService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/receipts', 'receiptconstructordb');
  ...
  protected convertDateFromClient<T extends IReceipt | NewReceipt | PartialUpdateReceipt>(receipt: T): RestOf<T> {
    return {
      ...receipt,
      date: receipt.date?.format(DATE_FORMAT) ?? null,
    };
  }
